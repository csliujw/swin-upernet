...
eval_interp = torch.nn.Upsample(size=(1024, 1024), mode='bilinear', align_corners=True)
...
def main():
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        if i_iter < cfg.WARMUP_STEP:
            ...
            main_pred_target,_ = model(images_t.cuda())
            ...
            loss_aux = loss_calc(aux_pred_source, labels_s['cls'].cuda()) * cfg.SOURCE_LOSS_WEIGHT + loss_calc(main_pred_target, labels_t['cls'].cuda()) * cfg.PSEUDO_LOSS_WEIGHT
            optimizer.zero_grad()
            loss = loss_main + loss_aux*0.4
            loss.backward()
            clip_grad.clip_grad_norm_(filter(lambda p: p.requires_grad, model.parameters()), max_norm=35, norm_type=2)
...
def val(model, targetloader, save_round_eval_path, cfg):
    ...
    with torch.no_grad():
        for batch in tqdm(targetloader):
            ...
            output = eval_interp(output).softmax(dim=1)
