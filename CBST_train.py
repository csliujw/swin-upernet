...
def main():
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        if i_iter < cfg.WARMUP_STEP:
            ...
            images_s, labels_s = batch[0]
            main_pred_source,_ = model(images_s.cuda())
            ...
            loss_aux = loss_calc(aux_pred_target, labels_s['cls'].cuda()) * cfg.SOURCE_LOSS_WEIGHT + loss_calc(aux_pred_target, labels_t['cls'].cuda()) * cfg.PSEUDO_LOSS_WEIGHT
            optimizer.zero_grad()
            loss = loss_main+loss_aux*0.4
            loss.backward()
            clip_grad.clip_grad_norm_(filter(lambda p: p.requires_grad, model.parameters()), max_norm=35, norm_type=2)
